...
pub async fn get_preview_image(
    ...
    let preview_path = file_service.get_preview_dir().join(format!("{}_{}.jpg", path.filename, path.page));
...
pub async fn get_pdf_preview(
    ...
    match page_opt {
        Some(page) => {
            let preview_path = file_service.generate_preview(&file_or_image, page)
                .map_err(|e| {
...
pub async fn generate_all_previews(
    ...
    let total_pages = metadata.lines()
    ...
    tokio::spawn(async move {
        ...
        info!("[Thread {:?}] Starting preview generation for {} ({} pages)", thread_id, file_clone, total_pages);
        
        for page in 1..=total_pages {
            info!("[Thread {:?}] Generating preview for {} - page {}/{}", thread_id, file_clone, page, total_pages);
            match file_service.generate_preview(&file_clone, page) {
                Ok(_) => {
                    info!("[Thread {:?}] Successfully generated preview for {} - page {}/{}", thread_id, file_clone, page, total_pages);
                    progress_clone.processed_pages.fetch_add(1, Ordering::Relaxed);
                },
                Err(e) => error!("[Thread {:?}] Failed to generate preview for {} page {}/{}: {}", thread_id, file_clone, page, total_pages, e),
            }
        }
        progress_clone.is_complete.store(1, Ordering::Relaxed);
        info!("[Thread {:?}] Finished generating all previews for {} ({} pages total)", thread_id, file_clone, total_pages);
    });
